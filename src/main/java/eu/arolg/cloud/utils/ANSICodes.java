package eu.arolg.cloud.utils;

public class ANSICodes {
    public static String RESET = "\u001B[0m";

    public static String BOLD = "\u001B[1m";
    public static String UNDERLINE = "\u001B[4m";
    public static String REVERSED = "\u001B[7m";

    public static String BLACK = "\u001B[30m";
    public static String RED = "\u001B[31m";
    public static String GREEN = "\u001B[32m";
    public static String YELLOW = "\u001B[33m";
    public static String BLUE = "\u001B[34m";
    public static String MAGENTA = "\u001B[35m";
    public static String CYAN = "\u001B[36m";
    public static String WHITE = "\u001B[37m";

    public static String BRIGHT_BLACK = "\u001B[30;1m";
    public static String BRIGHT_RED = "\u001B[31;1m";
    public static String BRIGHT_GREEN = "\u001B[32;1m";
    public static String BRIGHT_YELLOW = "\u001B[33;1m";
    public static String BRIGHT_BLUE = "\u001B[34;1m";
    public static String BRIGHT_MAGENTA = "\u001B[35;1m";
    public static String BRIGHT_CYAN = "\u001B[36;1m";
    public static String BRIGHT_WHITE = "\u001B[37;1m";

    public static String BLACK_BG = "\u001B[40m";
    public static String RED_BG = "\u001B[41m";
    public static String GREEN_BG = "\u001B[42m";
    public static String YELLOW_BG = "\u001B[43m";
    public static String BLUE_BG = "\u001B[44m";
    public static String MAGENTA_BG = "\u001B[45m";
    public static String CYAN_BG = "\u001B[46m";
    public static String WHITE_BG = "\u001B[47m";

    public static String BRIGHT_BLACK_BG = "\u001B[40;1m";
    public static String BRIGHT_RED_BG = "\u001B[41;1m";
    public static String BRIGHT_GREEN_BG = "\u001B[42;1m";
    public static String BRIGHT_YELLOW_BG = "\u001B[43;1m";
    public static String BRIGHT_BLUE_BG = "\u001B[44;1m";
    public static String BRIGHT_MAGENTA_BG = "\u001B[45;1m";
    public static String BRIGHT_CYAN_BG = "\u001B[46;1m";
    public static String BRIGHT_WHITE_BG = "\u001B[47;1m";
}
